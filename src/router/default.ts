import type {IRule} from "@/domain/iRule.ts";

const defaultRoute: IRule[] = [
    {
        id: 1,
        pid: 0,
        type: 'menu',
        name: '仪表盘',
        local: "menu.dashboard",
        key: "dashboard",
        icon: "DashboardOutlined",
    },
    {
        id: 2,
        pid: 1,
        type: 'route',
        name: '分析页',
        local: "menu.analysis",
        icon: "PieChartOutlined",
        key: 'dashboard.analysis',
        path: '/dashboard/analysis',
        elementPath: '/dashboard/analysis',
    },
    {
        id: 3,
        pid: 1,
        type: 'route',
        name: '监控页',
        local: "menu.monitor",
        icon: "BarChartOutlined",
        key: 'dashboard.monitor',
        path: '/dashboard/monitor',
        elementPath: '/dashboard/monitor',
    },
    {
        id: 4,
        pid: 1,
        type: 'route',
        name: '工作台',
        local: "menu.workplace",
        icon: "LineChartOutlined",
        key: 'dashboard.workplace',
        path: '/dashboard/workplace',
        elementPath: '/dashboard/workplace',
    },
    {
        id: 5,
        pid: 0,
        type: 'menu',
        name: '结果页',
        local: "menu.result",
        key: "result",
        icon: "AppstoreOutlined",
    },
    {
        id: 6,
        pid: 5,
        type: 'route',
        name: '成功页',
        local: "menu.result.success",
        icon: "CheckCircleOutlined",
        key: 'result.success',
        path: '/result/success',
        elementPath: '/result/success',
    },
    {
        id: 7,
        pid: 5,
        type: 'route',
        name: '失败页',
        local: "menu.result.fail",
        icon: "CloseCircleOutlined",
        key: 'result.fail',
        path: '/result/fail',
        elementPath: '/result/fail',
    },
    {
        id: 8,
        pid: 5,
        type: 'route',
        name: '警告页',
        local: "menu.result.warning",
        icon: "WarningOutlined",
        key: 'result.warning',
        path: '/result/warning',
        elementPath: '/result/warning',
    },
    {
        id: 9,
        pid: 5,
        type: 'route',
        name: '信息页',
        local: "menu.result.info",
        icon: "ExclamationCircleOutlined",
        key: 'result.info',
        path: '/result/info',
        elementPath: '/result/info',
    },
    {
        id: 10,
        pid: 5,
        type: 'route',
        name: '403',
        local: "menu.result.403",
        icon: "AppstoreOutlined",
        key: 'result.403',
        path: '/result/403',
        elementPath: '/result/403',
    },
    {
        id: 11,
        pid: 5,
        type: 'route',
        name: '404',
        local: "menu.result.404",
        icon: "AppstoreOutlined",
        key: 'result.404',
        path: '/result/404',
        elementPath: '/result/404',
    },
    {
        id: 12,
        pid: 5,
        type: 'route',
        name: '500',
        local: "menu.result.500",
        icon: "AppstoreOutlined",
        key: 'result.500',
        path: '/result/500',
        elementPath: '/result/500',
    },
    {
        id: 13,
        pid: 0,
        type: 'menu',
        name: '权限管理',
        local: "menu.auth",
        icon: "SafetyCertificateOutlined",
        key: 'auth',
    },
    {
        id: 14,
        pid: 13,
        type: 'route',
        name: '页面权限',
        local: "menu.auth.page",
        icon: "AppstoreOutlined",
        key: 'auth.page',
        path: '/auth/page',
        elementPath: '/auth/page',
    },
    {
        id: 15,
        pid: 13,
        type: 'route',
        name: '按钮权限',
        local: "menu.auth.button",
        icon: "AppstoreOutlined",
        key: 'auth.button',
        path: '/auth/button',
        elementPath: '/auth/button',
    },
    {
        id: 16,
        pid: 15,
        type: 'rule',
        name: '新增权限',
        key: 'auth.button.create',
    },
    {
        id: 17,
        pid: 15,
        type: 'rule',
        name: '编辑权限',
        key: 'auth.button.update',
    },
    {
        id: 18,
        pid: 15,
        type: 'rule',
        name: '删除权限',
        key: 'auth.button.delete',
    },
    {
        id: 19,
        pid: 15,
        type: 'rule',
        name: '新增权限',
        key: 'auth.button.query',
    },
    {
        id: 20,
        pid: 0,
        type: 'menu',
        name: '多级菜单',
        local: "menu.multi-menu",
        icon: "MenuOutlined",
        key: 'multi-menu',
    },
    {
        id: 21,
        pid: 20,
        type: 'menu',
        name: '二级菜单',
        local: "menu.multi-menu.two",
        icon: "MenuOutlined",
        key: 'multi-menu.two',
    },
    {
        id: 22,
        pid: 20,
        type: 'route',
        name: '二级页面',
        local: "menu.multi-menu.first",
        icon: "MenuOutlined",
        key: 'multi-menu.first',
        path: '/multi-menu/first',
        elementPath: '/multi-menu/first',
    },
    {
        id: 23,
        pid: 21,
        type: 'menu',
        name: '三级菜单',
        local: "menu.multi-menu.two.three",
        icon: "MenuOutlined",
        key: 'multi-menu.two.three',
    },
    {
        id: 24,
        pid: 21,
        type: 'route',
        name: '三级页面',
        local: "menu.multi-menu.two.second",
        icon: "MenuOutlined",
        key: 'multi-menu.two.second',
        path: '/multi-menu/two/second',
        elementPath: '/multi-menu/second',
    },
    {
        id: 25,
        pid: 23,
        type: 'route',
        name: '四级页面',
        local: "menu.multi-menu.two.three.third",
        icon: "MenuOutlined",
        key: 'multi-menu.two.three.third',
        path: '/multi-menu/two/three/third',
        elementPath: '/multi-menu/third',
    },
    {
        id: 26,
        pid: 0,
        type: 'menu',
        name: '页面布局',
        local: "menu.page-layout",
        icon: "LayoutOutlined",
        key: 'page-layout',
    },
    {
        id: 27,
        pid: 26,
        type: 'route',
        name: '基础布局',
        local: "menu.page-layout.base-layout",
        icon: "AppstoreOutlined",
        key: 'page-layout.base-layout',
        path: '/page-layout/base-layout',
        elementPath: '/page-layout/base-layout',
    },
    {
        id: 28,
        pid: 26,
        type: 'route',
        name: '固定头部',
        local: "menu.page-layout.fix-header",
        icon: "AppstoreOutlined",
        key: 'page-layout.fix-header',
        path: '/page-layout/fix-header',
        elementPath: '/page-layout/fix-header',
    },
    {
        id: 29,
        pid: 26,
        type: 'route',
        name: '页面描述',
        local: "menu.page-layout.descriptions",
        icon: "AppstoreOutlined",
        key: 'page-layout.descriptions',
        path: '/page-layout/descriptions',
        elementPath: '/page-layout/descriptions',
    },
    {
        id: 30,
        pid: 0,
        type: 'menu',
        name: '个人中心',
        local: "menu.user",
        icon: "UserOutlined",
        key: 'user',
    },
    {
        id: 31,
        pid: 30,
        type: 'route',
        name: '用户信息',
        local: "menu.user.info",
        icon: "AppstoreOutlined",
        key: 'user.info',
        path: '/user/info',
        elementPath: '/user/info/index',
    },
    {
        id: 33,
        pid: 31,
        type: 'nested-route',
        name: '我的文章',
        key: 'user.info.article',
        path: '/user/info',
        elementPath: '/user/info/article',
    },
    {
        id: 34,
        pid: 31,
        type: 'nested-route',
        name: '我的动态',
        key: 'user.info.condition',
        path: '/user/info/condition',
        elementPath: '/user/info/condition',
    },
    {
        id: 34,
        pid: 31,
        type: 'nested-route',
        name: '站内通知',
        key: 'user.info.notice',
        path: '/user/info/notice',
        elementPath: '/user/info/notice',
    },
    {
        id: 32,
        pid: 30,
        type: 'route',
        name: '用户设置',
        local: "menu.user.setting",
        icon: "AppstoreOutlined",
        key: 'user.setting',
        path: '/user/setting',
        elementPath: '/user/setting',
    },
]

export default defaultRoute
